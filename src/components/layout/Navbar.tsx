...
import { Menu, MessageCircle, LayoutDashboard, Home, ShoppingBag, Image, Star, MapPin, Phone, Layers, ChevronDown } from 'lucide-react'
          ...
          <nav className="hidden lg:flex items-center gap-0.5">
            {PRIMARY_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                id={link.href === '/frame-designer' ? 'frame-designer-link' : undefined}
                className={cn(
              ...
              >
                {MORE_LINKS.map((link) => (
                  <DropdownMenuItem key={link.href} className="p-0 focus:bg-transparent">
                    <Link
                      href={link.href}
                      id={link.href === '/track-order' ? 'track-order-link' : undefined}
                      className={cn(
